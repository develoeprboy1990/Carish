...
class SearchController extends Controller
{
  ...
  public function autoPartsListing($params = null)
  {
   ...
   if(isset($parent_child_cat[0]) && isset($parent_child_cat[1]) && strpos($parent_child_cat[1], 'dealer_') === false && strpos($parent_child_cat[1], 'partid_') === false && strpos($parent_child_cat[1], 'city_') === false && strpos($parent_child_cat[1], 'sortby_') === false && strpos($parent_child_cat[1], 'productcode_') === false && strpos($parent_child_cat[1], 'condition_') === false)
   {
    ...
    if (Arr::has($queyStringArray, 'make')) {

      if (Arr::has($queyStringArray, 'make')) {
        $make_id   = $queyStringArray['make'];
      }
      
      $make_id        = array_unique($make_id);
      $sparepartsQuery->whereIn('make_id', $make_id);

      $makeName  = make::select('title')->whereIn('id', $make_id)->where('status', 1)->get();
      // $cityName  = $cityName->name;
      $list      = "'" . implode("', '", $make_id) . "'";
      $queryStringArray['makes'] = " AND m.make_id IN(" . $list . ")";
    }
...
*/
  ...
  public function simpleSearch($params)
  {
    ...
    $activeLanguage  = \Session::get('language');
    $search_page_title =  Lang::get('header.car_search');
    ...
    if (Arr::has($quey_string_slug, 'isf')) {
      ...
      $search_page_title     = Lang::get('header.featured_used_cars');
    }
  ...
  public function getSpModels($id)
  {
      ...
      $html = '';
      $html .= '<option value="" >'.Lang::get('ads.select_one').'</option>';
