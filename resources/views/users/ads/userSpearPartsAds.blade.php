              ...
              <div class="tab-pane active" id="ad-tab2">
                <div class="align-items-center d-flex justify-content-between mb-md-3 mb-2">
                  <div class="display-ad-title">
                    <h6 class="mb-0 font-weight-normal">{{__('ads.displaying')}} <span id="ads_count">{{@$PartsAdsCount->count()}}</span> {{__('ads.ad_listing')}}</h6>
                  </div>
                  @php
                  $status = 1;
                  @endphp
                  ...
                  <div class="display-ad-status">
                    <select name="" class="form-control form-control-sm parts_selector">
                      <option value="1" @if($status=='1' ) selected @endif>{{__('ads.active')}} ({{@$PartsAds->count()}})</option>
                      <option value="0" @if($status=='0' ) selected @endif>{{__('ads.Pending')}} ({{@$pending_ads->count()}})</option>
                      <option value="2" @if($status=='2' ) selected @endif>{{__('ads.removed')}} ({{@$removed_ads->count()}})</option>
                    </select>
                  </div>
                </div>
                  ...
                  <div class="ads-listing-rows filtered_ads">
                    ...
                    <div class="row p-3 mx-0">
                      ...
                      <div class="col-lg-9 col-sm-8 col-8 pl-md-0 pl-sm-3 pl-0 pr-0 ads-listing-text">
                        ...
                        <p class="ads-views mb-0"><em class="fa fa-eye"></em> {{@$p_ad->views}} {{__('ads.views')}}</p>
                        @if($p_ad->status == 1)
                        <p class="ads-views mb-0" style="color:#005e9b;"><strong>{{__('ads.active_until')}}</strong> {{$p_ad->active_until != null ? $p_ad->active_until : 'N.A'}}</p>
                        @endif
                      </div>
                        ...
                        <div class="row align-items-sm-center">
                          <div class="col-lg-7 col-md-8 col-sm-8 mb-0 editProfile actionbtn ">
                            <ul class="list-unstyled mb-0">
                              <li class="list-inline-item"><a target="" href="{{url('user/spare-part-ad/'.@$p_ad->id)}}" class="border d-inline-block px-sm-3 px-2 py-1 rounded">{{__('ads.edit')}} <em class=" ml-1 fa fa-pencil"></em></a></li>

                              <li class="list-inline-item"><a href="" class="border d-inline-block px-sm-3 px-2 py-1 rounded remove-ad" data-id="{{$p_ad->id}}">{{__('ads.remove')}} <em class="ml-1 fa fa-trash"></em></a></li>
                          ...
                          <div class="col-lg-5 col-md-4 col-sm-4 pt-3 pt-sm-0 featuredAd text-sm-right">
                            ...
                            <a href="javascript:void(0)" data-id="{{@$p_ad->id}}" class="make_it_featured" data-toggle="modal" data-target="#featureModal"><em class="fa fa-star"></em> {{__('ads.feature_this_ad')}}</a>
                            @elseif(@$p_ad->is_featured == 'true')
                            <a href="javascript:void(0)" style="color: #0072BB;"><em class="fa fa-star"></em>
                              <span>{{__('common.featured_until')}} {{@$p_ad->feature_expiry_date !== null ? Carbon::parse(@$p_ad->feature_expiry_date)->format('d/m/Y H:i:s') : '--'}}</span></a>
                    ...
                    @else
                    <div class="row p-3 mx-0">
                      <h5>{{__('common.no_parts_founds')}}</h5>
                    </div>
                    @endif
                  </div>
                </div>
    ...
    <div class="modal-dialog modal-lg" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h2 class="modal-title" id="exampleModalLabel">{{__('spareparts.feature_spare_part')}}</h2>
        ...
        <form class="featured_request_form">
          ...
          <div class="modal-body">
            ...
            <div class="input-group" id="car_data" style="border: none;margin-top: 40px;margin-left: 30%;">
              ...
              <span style="background-color: white;width: 25%;border-top-left-radius: 5px;border-bottom-left-radius: 5px;border: 1px solid #aaa;border-right: none;padding-left: 5px;line-height: 2;font-weight: 600;">{{__('common.feature_this_sparepart_for')}} </span>
