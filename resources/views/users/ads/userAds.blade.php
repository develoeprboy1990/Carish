              ...
              <div class="tab-pane active" id="ad-tab1">
                <div class="align-items-center d-flex justify-content-between mb-md-3 mb-2">
                  <div class="display-ad-title">
                    <h6 class="mb-0 font-weight-normal">{{__('ads.displaying')}} <span id="ads_count">{{@$ads->count()}}</span> {{__('ads.ad_listing')}}</h6>
                  </div>
                  <div class="display-ad-status">
                    <select name="ads_selector" id="ads_selector" class="form-control ads_selector form-control-sm">
                      <option value="1">{{__('ads.active')}} ({{@$ads->count()}})</option>
                      <option value="0" {{@$find == 10 ? 'selected' : ''}}>{{__('ads.Pending')}} ({{@$pending_ads->count()}})</option>
                  ...
                  <div class="ads-listing-rows filtered_ads">
                    ...
                    <div class="row p-3 mx-0">
                      ...
                      <div class="col-lg-9 col-sm-8 col-8 pl-md-0 pl-sm-3 pl-0 pr-0 ads-listing-text">
                        ...
                        @if($ad->status == 1)
                        <p class="ads-views mb-0" style="color:#005e9b;"><strong>{{__('ads.active_until')}}</strong> {{$ad->active_until != null ? $ad->active_until : 'N.A'}}</p>
                        @endif
                      </div>
                        ...
                        <div class="row align-items-sm-center">
                          <div class="col-lg-7 col-md-8 col-sm-8 mb-0 editProfile actionbtn ">
                            <ul class="list-unstyled mb-0">
                              @if($ad->status != 2)
                              <li class="list-inline-item"><a href="{{route('post.edit',['id'=>$ad->id])}}" class="border d-inline-block px-sm-3 px-2 py-1 rounded"> {{__('ads.edit')}} <em class=" ml-1 fa fa-pencil"></em></a></li>
                              @endif

                              <li class="list-inline-item"><a href="" class="border d-inline-block px-sm-3 px-2 py-1 rounded remove-ad" data-id="{{$ad->id}}"> {{__('ads.remove')}}<em class="ml-1 fa fa-trash"></em></a></li>

                              <li class="list-inline-item"><a href="#" class="d-none border px-sm-3 px-2 py-1 rounded">{{__('ads.inactive')}} <em class="ml-1 fa fa-refresh"></em></a></li>
                            </ul>
                          </div>
                          <div class="col-lg-5 col-md-4 col-sm-4 pt-3 pt-sm-0 featuredAd text-sm-right">
                            ...
                            <a href="javascript:void(0)" style="color: #0072BB;"><em class="fa fa-star"></em>
                              <span>{{__('common.featured_until')}}&nbsp &nbsp {{@$ad->feature_expiry_date !== null ? Carbon::parse(@$ad->feature_expiry_date)->format('d/m/Y H:i:s') : '--'}}</span></a>
    ...
    <div class="modal-dialog modal-lg" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h2 class="modal-title" id="exampleModalLabel">{{__('ads.feature_ad')}}</h2>
        ...
        <form class="featured_request_form">
          ...
          <div class="modal-body">
            ...
            <div class="input-group" id="car_data" style="border: none;margin-top: 40px;margin-left: 30%;">
              <span style="background-color: white;width: 17%;border-top-left-radius: 5px;border-bottom-left-radius: 5px;border: 1px solid #aaa;border-right: none;padding-left: 5px;line-height: 2;font-weight: 600;" class="feature_span">{{__('ads.feature_this_ad_for')}} </span>
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      ...
      $(document).on('click', '.resubmit-ad', function(e) {
        ...
        swal({
            title: "{{__('ads.you_sure')}}",
            text: "{{__('ads.you_sure_toreactive')}}",
            type: "error",
            showCancelButton: true,
            confirmButtonColor: "#DD6B55",
            confirmButtonText: "{{__('ads.yes_do')}}",
          ...
          function(isConfirm) {
            if (isConfirm) {
              $.ajax({
                ...
                success: function(data) {
                  if (data.error == false) {
                    toastr.success('Success!', "{{__('ads.reactive_success')}}", {
                      "positionClass": "toast-botom-right"
                    });
                    location.reload();
                  }
