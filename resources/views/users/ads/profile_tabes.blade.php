@section('title') {{ __('header.my_dashboard') }} @endsection
...
<p class="mb-sm-2 mb-1 font-weight-semibold member-time">{{__('profile.member_since')}} {{date('M dS, Y', strtotime($loggedInUser->created_at))}}</p>
<ul class="list-unstyled themecolor font-weight-semibold mb-0">
   @if($loggedInUser->customer_role == 'individual')
 <li class="list-inline-item"><a target="" href="{{route('change.profile')}}">{{__('profile.edit_profile')}}</a></li>
 @else
 <li class="list-inline-item"><a target="" href="{{route('my.business.profile')}}">{{__('profile.edit_profile')}}</a></li>
 @endif
 <li class="list-inline-item"><a target="" href="{{route('change-password')}}">{{__('profile.change_password')}}</a></li>
</ul>
</div>         
</div> 
...
<li class="nav-item">
  <a class="nav-link {{ (request()->is('user/my-ads') || request()->is('user/my-spear-parts-ads') || request()->is('user/my-services-ads')) ? 'active' : '' }}" href="{{route('my-ads')}}">
    <em class="fa mr-lg-2 mr-1 fa-bullhorn"></em>{{__('header.my_ads')}}</a>
</li>
<li class="nav-item">
  <a class="nav-link {{ (request()->is('user/my-saved-ads')) ? 'active' : '' }}" href="{{route('my-saved-ads')}}">
  <em class="fa mr-lg-2 mr-1 fa-heart"></em> {{__('header.my_save_ads')}}</a>
</li>
<li class="nav-item">
  <a class="nav-link {{ (request()->is('user/my-alerts')) ? 'active' : '' }}"  href="{{route('my-alerts')}}">
  <em class="fa mr-lg-2 mr-1 fa-bell-o"></em> {{__('header.alerts')}}</a>
</li>
 <li class="nav-item">
  <a class="nav-link {{ (request()->is('user/my-messages')) ? 'active' : '' }}" href="{{route('my-messages')}}">
  <em class="fa mr-lg-2 mr-1 fa-envelope-o"></em> {{__('header.messages')}} @if(@$unread_msgs > 0) ({{@$unread_msgs}}) @endif </a>
</li>
 <li class="nav-item">
  <a class="nav-link {{ (request()->is('user/change-password')) ? 'active' : '' }}"  href="{{route('change-password')}}">
  <em class="fa mr-lg-2 mr-1 fa-unlock-alt"></em> {{__('header.change_password')}}</a>
</li>
 <li class="nav-item">
  <a class="nav-link {{ (request()->is('user/my-payment')) ? 'active' : '' }}"  href="{{route('my-payment')}}">
    <em class="fa mr-lg-2 mr-1 fa-credit-card"></em> {{__('header.payment')}}</a>
</li>
</ul>
</div>