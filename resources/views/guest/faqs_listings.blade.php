@extends('layouts.app')
@section('title') {{ __('header.faq') }} @endsection
@push('styles')
<style type="text/css">
  .expand_success{
    background: #ddd;
...
header Ends here -->
      <div class="internal-page-content mt-4 pb-0 sects-bg">
        <div class="bgcolcor bgcolor1 internal-banner text-white cc-page-banner">
          <div class="container">
            <h3 class="font-weight-semibold text-capitalize mb-1">{{__('header.faqs')}}</h3>
          </div>
        </div>
        <div class="container mt-n5">
          ...
          <form class="p-md-2" action="{{route('faqs.search')}}">
              <div class="form-row row">
                ...
                <input type="text" name="term" id="term" value="{{@$term}}" placeholder="{{__('common.search')}} {{__('header.faqs')}}" class="form-control">
                </div>
                <div class="col-5 col-md-3 col-sm-4 form-group mb-0">
                  <input type="submit" value="{{__('common.search')}}" class="btn  themebtn3 w-100 pl-3 pr-3">
                </div>
              </div>
            </form>
          </div>
...
<div class="bg-white border p-md-4 p-3">
  <h4>{{__('common.no_record_found')}}</h4>
</div>
              @endif
