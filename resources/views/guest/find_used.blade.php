@extends('layouts.app')
@section('title') {{ __('header.find_used_cars') }} @endsection
@push('styles')
...
$("#selectCity").select2({
  language: {
    searching: function() {
      return "{{__('search.searching')}}";
    },
    noResults: function(){
      return "{{__('search.no_result_found')}}";
    }
  },
                ajax: {
   ...
   /* NEW SEARCH STRING */
    $(".search_check_submit").click(function(e) { 
        if(e.preventDefault){
          ...
          {
            alert("{{ __('common.sorry_invalid_mileage_range') }}");
            return false;
          }
          if(fromMillage != '' || toMillage != ''){
 ...
 @endphp
   <div class="internal-page-content mt-4 sects-bg pb-0">
        <div class="bgcolcor bgcolor1 internal-banner text-white cc-page-banner">
          <div class="container">
            <h2 class="font-weight-semibold  mb-1">{{__('search.findcarsforsalein')}} {{__('search.estonia')}}</h2>
            <p class="mb-0">{{__('search.withthousandofcarswehave')}}</p>
          </div>
        </div>
      ...
      <div class="col-12 form-group mb-0 pt-sm-2 mt-4">
        ...
        <select id="transmission" name="transmission" class="form-control search_selectbox_submit">
              <option value="" disabled="true" selected>{{ __('home.alltransmissiontypes') }}</option>
          ...
          <div class="col-lg-4 col-md-4 col-sm-6 col-12 form-group">
             <label class="font-weight-semibold">{{ __('home.all_body_types') }}</label>
              ...
              <option value="" disabled="true">{{ __('home.all_body_types') }}</option> 
            </select>
          </div>
          <div class="col-lg-4 col-md-4 col-sm-6 col-12 form-group">
        ...
        <div class="form-row">
          <div class="col-lg-4 col-md-4 col-sm-6 col-12 form-group">
           ...
           <select id="picture" name="have_picture" class="form-control search_selectbox_submit">
              <option value="" disabled="true" selected>{{ __('home.without_picture') }} </option>
              <option value="pic_Ads-With-Pictures">{{ __('home.with_picture') }} </option>
            </select>
          </div>
        <div class="col-lg-4 col-md-4 col-sm-6 col-12 form-group">
           <label class="font-weight-semibold">{{ __('search.seller_type') }}</label>
            <select id="ads_type" name="ads_type" class="form-control search_selectbox_submit">
              <option value="" disabled="true"  selected>{{ __('search.seller_type') }} </option>
              <option value="seller_business">{{ __('search.dealers') }}</option>
              <option value="seller_individual">{{ __('search.individuals') }}</option>
            </select>
          </div>
          <div class="col-lg-4 col-md-4 col-sm-6 col-12 form-group">
            ...
            <select id="featured" name="is_featured" class="form-control search_selectbox_submit">
              <option value="" disabled="true" selected>{{__('home.all_ad_types')}}</option>
...
<p>{{ __('home.post_your_car_ad') }} <a href="{{route('faqs')}}" class="themecolor"><u>{{ __('home.for_free') }}</u></a> {{ __('home.in_few_seconds') }} </p>
...
<div class="col-12 mt-4 pt-3">
  <a target="" href="{{url('user/post-car-ad')}}" class="btn  themebtn1">{{__('common.sell_your_car')}}</a>
</div>
</div>
...
<div class="internal-sect-title-col align-items-end d-md-flex justify-content-between mb-4">
  <h3 class="internal-sect-title font-weight-semibold mb-0">{{__('home.featured_used')}} {{trans_choice('cars',0)}}</h3>
  <div class="ml-md-3 text-right view-all-link">
    <a target="" href="{{route('simple.search')}}/isf_featured" class="themecolor">{{__('home.viewallfeaturedusedcars')}}</a>
  </div>
</div>
  ...
  <div class="item item two-row-item">
    ...
    <div class="box-shadow">
      ...
      @endif
        ...
        <span class="bgcolor2 font-weight-semibold pb-1 pl-2 pr-2 pt-1  text-white d-inline-block">{{__('home.featured')}}</span>
        </figcaption>
      </figure>
        ...
        <div class="container pt-3">
          <div class="browse-car-by-sect bg-white border mt-md-5 mt-4">
            <div class="bcb-title f-size1 p-md-4 m-md-2  p-3">
              <h3>{{__('common.browse_cars_by_make')}}</h3>
              <p class="mb-0"><!-- 444,337 -->{{__('common.used_cars_available_in_estonia')}}</p>
            </div>
            
            <ul class="nav nav-justified nav-tabs browse-car-tabs">
              <li class="nav-item">
                <a class="nav-link active" data-toggle="tab" href="#bcb-tab1" role="tab" aria-controls="bcb-tab1" aria-selected="true">{{__('common.category')}}</a>
              </li>
              <li class="nav-item">
                <a class="nav-link" data-toggle="tab" href="#bcb-tab2" role="tab" aria-controls="bcb-tab2" aria-selected="true">{{__('common.make')}}</a>
              </li>
              <li class="nav-item">
                <a class="nav-link" data-toggle="tab" href="#bcb-tab3" role="tab" aria-controls="bcb-tab3" aria-selected="true">{{__('common.city')}}</a>
              </li>
              <li class="nav-item">
                <a class="nav-link" data-toggle="tab" href="#bcb-tab4" role="tab" aria-controls="bcb-tab4" aria-selected="true">{{__('home.body_type')}}</a>
              </li>
            </ul>
          ...
          <div class="row mt-5 featured-dealers-row">
            <div class="col-12">
              <div class="internal-sect-title-col align-items-end d-md-flex justify-content-between">
                <h3 class="internal-sect-title font-weight-semibold mb-0">{{__('search.featured')}} {{__('common.dealers')}} </h3>
                <div class="ml-md-3 text-right view-all-link">
                  <a href="{{ url('allservices/listing/ps_1') }}" class="f-size1 themecolor">{{__('home.view_all')}}  {{__('search.featured')}} {{__('common.dealers')}}</a>
                </div>
              </div>
              <div class="align-items-center row text-center featured-dealers-col">
                ...
                @endforeach
                @else
                <h5 class="text-center col-lg-12 mt-5">{{ __('common.no_feature_dealer_found') }}</h5>
                @endif
              </div>
            </div>
          </div>
          ...
          <div class="container">
            <div class="align-items-center col-lg-9 d-sm-flex justify-content-around ml-auto mr-auto mb-4 mb-md-5 mt-4 mt-md-5 pl-0 postAdRow pr-0 mb-sm-0 mb-5">
              ...
              <div class="sellCarBtn">
                <a href="{{route('sellcar')}}" class="btn themebtn1">{{__('search.sell_your_car')}}</a>
              </div>
            </div>
          </div>z
        </div>
