...
<div class="alert alert-success" id="save-success" style="display: none;">
  <p align="center">{{ __('ads.ad_saved_successfully') }}</p>
</div>
@php
      ...
      <div class="col-12 pageTitle detailpageTitle mt-md-5 mt-4">
        <nav aria-label="breadcrumb" class="breadcrumb-menu">
          <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a target="" href="{{route('simple.search')}}/used-cars-for-sale/isf_featured">{{ __('ads.featured_cars') }}</a></li>
            @else
            <li class="breadcrumb-item"><a target="" href="{{route('simple.search')}}/isp_popular">{{ __('ads.popular_cars') }}</a></li>
            @endif
          ...
          <ul id="image-gallery" class="gallery list-unstyled cS-hidden">
            ...
            <li class="position-relative overflow-hidden rounded car_gallery" data-thumb="{{asset('public/uploads/ad_pictures/cars/'.$ads->id.'/'.$ad_image->img)}}" data-src="{{asset('public/uploads/ad_pictures/cars/'.$ads->id.'/'.$ad_image->img)}}">
              <figcaption class="position-absolute bottom left right top d-flex align-items-start flex-column justify-content-between">
                @if(@$ads->is_featured == 'true')
                <span class="featuredlabel bgcolor2 font-weight-semibold pb-1 pl-2 pr-2 pt-1 text-uppercase text-white d-inline-block">{{ __('search.featured') }}</span>
                @endif
          ...
          <div class="seller-desc p-lg-4 p-3 mt-3">
            <div class="d-flex justify-content-between sellerinfo">
              <div class="sellerinfo-left">
                <h6 class="mb-4">{{ __('ads.seller_info') }}</h6>
                @if(@$ads->customer->login_status == '1')
                <p class="bestSeller font-weight-semibold mb-1">
                  <em class="fa fa-star"></em>{{ __('ads.trusted_seller') }}<em class="fa fa-star"></em>
                </p>
                @endif
                <ul class="list-unstyled">
                  <li><strong>{{ __('ads.dealer') }}:</strong>
                  ...
                  <li><strong>{{ __('common.address') }}:</strong> <a href="javascript:void(0)">{{$ads->customer->customer_default_address}}</a></li>
                  @else
                  <li><strong>{{ __('common.address') }}:</strong> <a href="javascript:void(0)">{{$ads->customer->city->name}}</a></li>
                  @endif
                </ul>
              </div>
              <div class="sellerinfo-right pl-3 text-right">
                ...
                <p class="incltext mb-0"> {{(@$ads->vat != null) ? __('common.incl_vat') : ''}}</p>
                <p class="themecolor2 font-weight-semibold mb-0 negotiable">{{(@$ads->neg != null) ? __('common.negotiable') : ''}}</p>
              </div>
            </div>
            <div class="align-items-center d-lg-flex d-md-block d-flex justify-content-between sellerContact">
              <div class="mb-md-3 m-lg-0 mb-0 sellerContact-left">
                ...
                <strong>{{ __('ads.view_more_ads_by') }} </strong>
                @if($ads->customer->customer_role == 'individual')
              ...
              <div class="pl-0 pl-lg-3 pl-md-0 pl-sm-3 sellerContact-right text-lg-right text-md-left text-right">
                <a href="javascript:void(0)" class="btn themebtn3 toggle_number">
                  <span class="d-flex align-items-center">
                    ...
                    <span class="half_number text-left">
                      {{substr(@$ads->customer->customers_telephone,0,7)}}....<br>{{ __('ads.show_number') }}
                    </span>
                ...
                <a href="javascript:void(0)" class="btn btn-transparent mt-lg-3 mt-md-0 mt-2 mt-0 send-msg-btn"><em class="fa fa-envelope"></em> {{ __('ads.send_message') }}</a>
                @else
                @if(empty(Auth::guard('customer')->user()) || ($ads->customer_id !=Auth::guard('customer')->user()->id))
                <a target="" href="{{url('user/login')}}" class="btn btn-transparent mt-lg-3 mt-md-0 mt-2 mt-0"><em class="fa fa-envelope"></em> {{ __('ads.send_message') }}</a>
                @endif
                @endif
              </div>
            </div>
            ...
            <ul class="list-unstyled sharelist font-weight-semibold mb-0" style="position: absolute;bottom: 10px;">
              ...
              @php $id = 'heart'; @endphp
              @endif
              <li class="list-inline-item mt-2"><a href="javascript:void(0)" class="saveAd saveAdBtn" data-id="{{$ads->id}}"><em id="{{$id}}" class="fa fa-heart"></em> {{__('search.save')}}</a></li>
              @else
              @if(@$checkSaved != null)
              ...
              @endif
              <li class="list-inline-item mt-2"><a href="javascript:void(0)" class="saveAdCok saveAdBtn" data-id="{{$ads->id}}"><em id="heart3" class="fa fa-heart"></em> {{__('search.save')}}</a></li>
              @endif
            </ul>
          </div>
        </div>
    ...
    <div class="row carinfoTabs">
      <div class="col-12 carinfoTabs-col text-center ">
        <ul class="bgcolor1 list-unstyled mb-0 nav nav-justified text-white">
          <li class="nav-item active"><a href="#carfeatures-sect" class="gotosect nav-link">{{ __('ads.car_features') }}</a></li>
          <li class="nav-item"><a href="#seller-comments-sect" class="gotosect nav-link">{{ __('ads.seller_comments') }}</a></li>
          <li class="nav-item"><a href="#similar-ads-sect" class="gotosect nav-link">{{ __('ads.similar_ads') }}</a></li>
        </ul>
      </div>
    </div>
  </div>
  <div class="carinfodetail-section bg-white mt-5 pt-5 pb-5">
      ...
      <div class="carinfodetail-row card-columns">
        ...
        <div class="card carfeatureinfo" id="carfeatures-sect">
          <h5 class="text-capitalize carInfotitle font-weight-bold">
            <img src="{{url('public/assets/img/detail-Car-features.jpg')}}" alt="icon">{{ __('ads.car_features') }}
          </h5>
          ...
          <div class="text-right">
            <a href="javascript:void(0)" class="themecolor font-weight-semibold">{{ __('ads.show_all') }}</a>
          </div>
          @endif
        </div>
        @endif
        <div class="card cardatainfo">
          <h5 class="carInfotitle text-capitalize">
            <img src="{{url('public/assets/img/detail-Car-data.jpg')}}" alt="icon">{{ __('ads.car_data') }}
          </h5>
          <table class="mb-0 table">
            <tbody>

              <tr>
                <td>{{ __('common.registration_number') }}:</td>
                <td>{{@$ads->car_number !== null ? @$ads->car_number : '--'}}</td>
              </tr>

              <tr>
                <td>{{ __('ads.buy_from') }}:</td>
                <td>
              ...
              <tr>
                <td>{{ __('ads.register_in_estonia') }}:</td>
                <td>{{!empty($ads->country_id) ? $ads->country->name : '--'}}</td>
              </tr>
              <tr>
                <td>{{ __('ads.register_in_estonia') }} {{!empty($ads->country_id) ? $ads->country->name : ''}}:</td>
                <td>{{!empty($ads->register_in_estonia) ? Carbon\Carbon::parse($ads->register_in_estonia)->format("m/Y") : '--'}}</td>
              </tr>
              <tr>
                <td colspan="2"> 
                  <a data-toggle="modal" data-target="#savedCarAd2" href="javascript:void()" class="view-more-ad themecolor" title="{{__('common.view_all')}}">{{__('common.view_all')}}</a>  
    ...
    <tr>
        ...
        <td>{{!empty($ads->register_in_estonia) ? Carbon\Carbon::parse($ads->register_in_estonia)->format("m/Y") : '--'}}</td>
                    ...
                    <button type="button"  class="close text-danger" data-dismiss="modal" aria-label="Close">{{__("ads.close")}}</button>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="card cardimensions">
          <h5 class="text-capitalize carInfotitle font-weight-bold">
            <img src="{{url('public/assets/img/detail-Car-dimensions.jpg')}}" alt="icon">{{ __('ads.dimensions') }}
          </h5>
        ...
        <div class="card careconomyinfo">
          <h5 class="text-capitalize carInfotitle font-weight-bold">
            <img src="{{url('public/assets/img/detail-Car-economy.jpg')}}" alt="icon">{{ __('ads.car_economy') }}
          </h5>
          <table class="mb-0 table table-borderless">
            <tbody>
              <tr>
                <td>{{ __('ads.average') }}</td>
      ...
      <div class="card cartagsinfo">
        <h5 class="text-capitalize carInfotitle font-weight-bold">
          <img src="{{url('public/assets/img/detail-Car-tags.jpg')}}" alt="icon">{{ __('ads.car_tags') }}
        </h5>
        <div class="car-tags">
          @if($ad_tags->count() > 0)
  ...
  <div class="  mt-4 mt-md-5 mb-4 mb-md-5">
    <div class="row seller-comments-row ml-auto mr-auto">
      <div class="col-12 seller-comments-col">
        <h5 class="text-capitalize carInfotitle font-weight-bold">
          <img src="{{url('public/assets/img/detail-Car-seller-coments.jpg')}}" alt="icon">{{ __('ads.seller_comments') }}
        </h5>
        ...
        <p class="mb-0">{{ __('ads.mention_site_at_sell') }}</p>
      </div>
    </div>
  </div>
</div>
<div class="container">
  <div class="align-items-center col-lg-9 d-sm-flex justify-content-around ml-auto mr-auto mb-4 mb-md-5 mt-4 mt-md-5 pl-0 postAdRow pr-0 pt-2 pt-sm-3  mb-sm-0 mb-5">
    ...
    <div class="sellCarBtn">
      <a target="" href="{{ route('sellcar') }}" class="btn themebtn1">{{ __('common.sell_your_car') }}</a>
    </div>
  </div>
  <div class="ml-0 mr-0 row bg-white" id="similar-ads-sect">
    <div class="col-12 p-4 similar-Ads-col">
      <h5 class="text-capitalize carInfotitle font-weight-bold">
        <img src="{{asset('public/assets/img/detail-Car-seller-coments.jpg')}}" alt="icon">{{ __('ads.similar_ads') }}
      </h5>
      <div class="row">
          ...
          <div class="box-shadow">
            <figure class="mb-0 position-relative text-left">
              ...
              <figcaption class="position-absolute top right left bottom">
                <span class="bgcolor2 d-inline-block featuredlabel font-weight-semibold mt-3 pb-1 pl-2 pr-2 pt-1 text-uppercase text-white">{{ __('search.featured') }}</span>
              </figcaption>
              @endif
            </figure>
        ...
        @else
        <div class="col-lg-3 col-md-6 col-6 mb-4 mb-lg-3 similar-post-col">
          <p><b>{{ __('ads.no_similar_ads_founds') }}</b></p>
        </div>
        @endif
      </div>
    </div>
    ...
    <div class="col-12 mt-lg-4 p-md-4 p-3 notifyForm detail-page-notifyform" style=" background: #E4E4E4;">
      <div class="row p-sm-2 align-items-center">
        <div class="col-lg-5 col-12 mb-lg-0 mb-3">
          ...
          <p class="mb-0">{{ __('ads.set_your_alerts_for') }} <strong>{{@$ads->maker->title}} {{@$ads->model->name}} {{@$ads->year}}</strong> {{ __('ads.and_we_will_email_you_relevant_ads') }}</p>
        </div>
          ...
          <form method="post" action="" id="make-car-alert">
            ...
            <div class="row form-row">
              <div class="col-sm-5 mb-md-0 mb-3 form-group">
                <input type="email" name="email" placeholder="{{__('common.type_our_email_address')}}" class="form-control" required>
              </div>
              ...
              <div class="col-sm-3 mb-md-0 mb-3 form-group notifySubCol">
                <input type="submit" value="{{ __('home.submit') }}" class="btn btn-block border-0 text-white notifySubBtn text-uppercase">
              </div>
            </div>
          </form>
        </div>
      ...
      <form action="{{route('send-message-to-customer')}}" method="post" class="send_msg_form">
        <div class="modal-body comp-modal-body pl-md-4 pr-md-4 pb-4">
          ...
          <div class="row">
            ...
            <div class="col-lg-7">
              <textarea class="form-control customer_message" name="customer_message" cols="8" rows="7" required style="resize: none;box-shadow: 0 0 2px 0 rgba(51,51,51,0.1);" id="ex3">{{ __('ads.hi_i_am_interested_in_your_car') }} <b>{{$ads->maker->title}} {{$ads->model->name}} {{@$ads->versions->name}} {{$ads->year}}</b> {{ __('ads.advertised_on') }} <a href="https://carish.ee/">Cairsh</a>. {{ __('ads.please_let_me_know_if_it_still_available') }}<br>Thanks.</textarea>
            </div>
          </div>
